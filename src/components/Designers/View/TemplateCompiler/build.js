...
function parseHTML (html, options) {
  ...
  while (html) {
    ...
    if (html === last) {
      ...
      if (process.env.NODE_ENV !== 'productionXX' && !stack.length && options.warn) {
...
function defineReactive (
  ...
  Object.defineProperty(obj, key, {
    ...
    set: function reactiveSetter (newVal) {
      ...
      if (process.env.NODE_ENV !== 'productionXX' && customSetter) {
        customSetter();
      }
      if (setter) {
...
function set (target, key, val) {
  ...
  if (target._isVue || (ob && ob.vmCount)) {
    process.env.NODE_ENV !== 'productionXX' && warn(
...
function mergeAssets (
  ...
  if (childVal) {
    process.env.NODE_ENV !== 'productionXX' && assertObjectType(key, childVal, vm);
...
strats.computed = function (
  ...
  if (childVal && process.env.NODE_ENV !== 'productionXX') {
    assertObjectType(key, childVal, vm);
  }
  if (!parentVal) { return childVal }
...
function transformNode (el, options) {
  ...
  if (process.env.NODE_ENV !== 'productionXX' && staticClass) {
  ...
  parseHTML(template, {
    ...
    start: function start (tag, attrs, unary) {
      ...
      if (isForbiddenTag(element) && !isServerRendering()) {
        ...
        process.env.NODE_ENV !== 'productionXX' && warn$1(
      ...
      if (!root) {
        ...
        } else if (process.env.NODE_ENV !== 'productionXX') {
          warnOnce(
...
function processKey (el) {
  ...
  if (exp) {
    if (process.env.NODE_ENV !== 'productionXX' && el.tag === 'template') {
...
function findPrevElement (children) {
  ...
  while (i--) {
    if (children[i].type === 1) {
      ...
      if (process.env.NODE_ENV !== 'productionXX' && children[i].text !== ' ') {
        warn$1(
...
function processSlot (el) {
  if (el.tag === 'slot') {
    ...
    if (process.env.NODE_ENV !== 'productionXX' && el.key) {
      warn$1(
    ...
    if (el.tag === 'template') {
      ...
      /* istanbul ignore if */
      if (process.env.NODE_ENV !== 'productionXX' && slotScope) {
        warn$1(
      ...
      /* istanbul ignore if */
      if (process.env.NODE_ENV !== 'productionXX' && el.attrsMap['v-for']) {
        warn$1(
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind
        ...
        if (process.env.NODE_ENV !== 'productionXX' && name === 'model') {
...
function on (el, dir) {
  if (process.env.NODE_ENV !== 'productionXX' && dir.modifiers) {
...
function genOnce (el, state) {
  ...
  if (el.if && !el.ifProcessed) {
    ...
    if (!key) {
      process.env.NODE_ENV !== 'productionXX' && state.warn(
