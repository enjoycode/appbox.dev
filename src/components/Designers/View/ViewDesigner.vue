    ...
    export default {
        ...
        methods: {
            ...
            initEditorCommands() {
                var _this = this
                // 切换编辑模式
                this.$refs.editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.KEY_1, function () {
                    _this.editMode = 'template'
                })
                this.$refs.editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.KEY_2, function () {
                    _this.editMode = 'script'
                })
                this.$refs.editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.KEY_3, function () {
                    _this.editMode = 'style'
                })
                this.$refs.editor.addCommand(window.monaco.KeyMod.CtrlCmd | window.monaco.KeyCode.KEY_4, function () {
            ...
            build() {
                ...
                // 先转换编译模版及脚本
                const compiledCode = compiler(this.typescriptServices, template, script, this.hashID, this.target.ID)
