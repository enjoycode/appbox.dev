...
export default class CellSelectionAdorner extends DesignAdorner {
    ...
    private DrawSelectedCells(g: CanvasRenderingContext2D,cells: Array<object>){
        var sx:number,sy:number,ex:number,ey:number;
        var sx2:number,sy2:number,ex2:number,ey2:number;        
        var draw: boolean =false;
        for (var i = 0; i < cells.length; i++) {
            var element = cells[i];
            if(element instanceof ReportItemDesigner){
                if (element.Parent === this.Target) {
                    //top
                    draw=true;
                    ...
                    for (var j = 0; j < cells.length; j++) {
                        if (i == j)
                            continue;                        
                        var comp = cells[j];
                        if (comp instanceof ReportItemDesigner) {
                            if (comp.Parent === this.Target) {
    ...
    public OnRender(g: CanvasRenderingContext2D) {
        ...
        //画选择的Cell
        var surface = this.Target.Surface as DesignSurface;
        g.lineWidth = 2;
        // for (var i = 0; i < surface.SelectionService.SelectedItems.length; i++) {
        //     var element = surface.SelectionService.SelectedItems[i];
        //     if (element.Parent === this.Target) {
        //         g.strokeRect(element.Bounds.X, element.Bounds.Y, element.Bounds.Width, element.Bounds.Height);
        //     }
        // }
        this.DrawSelectedCells(g,surface.SelectionService.SelectedItems)
    }

    //todo:参照TableDesignerBase.GetGlyphs()方法重构
