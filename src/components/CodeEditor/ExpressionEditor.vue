<template>
    <code-editor :height="height" ref="editor" language="csharp" :fileName="fileName"
                 @mounted="onEditorMounted"></code-editor>
</template>
...
export default {
    ...
    methods: {
        // 代码编辑器初始化后开始加载表达式代码
        onEditorMounted() {
            const args = [this.ownerType, this.ownerID, this.propertyName]
            $runtime.channel.invoke('sys.DesignService.OpenExpression', args).then(res => {
                this.onCodeLoaded(res)
            }).catch(err => this.$message.error('加载表达式错误: ' + err))
        },
        onCodeLoaded(res) {
        ...
        onCodeChanged(event) {
            ...
            for (var i = 0; i < event.changes.length; i++) {
                ...
                $runtime.channel.invoke('sys.DesignService.ChangeBuffer', [2, this.fileName,
        ...
        close(isCancel) {
            var _this = this
            $runtime.channel.invoke('sys.DesignService.CloseExpression', [this.fileName, isCancel]).catch(err => {
