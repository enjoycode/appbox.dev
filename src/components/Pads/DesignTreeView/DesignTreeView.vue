    ...
    <div class="pane">
        ...
        <div class="treewarp">
            <e-tree class="designTree" ref="designTree" dragdrop v-loading="loading" node-key="ID" :current-node-key="currentNodeKey"
                ...
                :filter-node-method="filterNode" :data="designNodes" :props="treeOption" highlight-current :default-expanded-keys="['DataStore','Applications']">
            </e-tree>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        watch: {
            keyword(val) {
                this.$refs.designTree.filter(val)
            }
        },
        methods: {
            ...
            // 取出设计树中的所有Entity并根据application分组
            // result为数组 然后传入
            loopGetEntityNodes(nodes, result) {
                var _that = this
                result = result || []
                nodes.forEach(function (node) {
                    var newNode
                    if (node.Type === DesignNodeType.ApplicationNode) {
                        newNode = _that.clone(node)
                        result.push(newNode)
                    }
                    ...
                    if ((node.Type === DesignNodeType.ApplicationRoot ||
                        node.Type === DesignNodeType.ApplicationNode ||
                        (node.Type === DesignNodeType.ModelRootNode && node.ID.split('-')[1] === '1') ||
                        node.Type === DesignNodeType.FolderNode) &&
                        node.Nodes.length > 0) {
                        _that.loopGetEntityNodes(node.Nodes, result)
                    }
                })
                return true
            },
            clone(node) { // 复制object数据 注意：此复制为浅复制 array不会复制
                var o = {}
                for (var key in node) {
