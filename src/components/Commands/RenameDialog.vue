<template>
    <el-dialog class="dialog" title="Rename" :visible.sync="visible" @close="onClose" :before-close="onClosing"
               width="450px">
        <div>
...
export default {
    ...
    methods: {
        ...
        onOkClick() {
            ...
            let args = [this.dlgProps.targetType, this.dlgProps.targetModel.ID, this.dlgProps.oldName, this.newName]
            $runtime.channel.invoke('sys.DesignService.Rename', args).then(res => {
                this.running = false
                this.caDisabled = false
                this.$message.success('Rename succeed')
                this.visible = false
                // 如果重命名模型，刷新模型显示文本
                if (this.dlgProps.renameModel) {
                    this.dlgProps.targetModel.Text = this.newName
                    store.designers.refreshTitle(this.dlgProps.targetModel)
                } else {
                    res.push(this.dlgProps.targetModel.ID) // 后端可能没有加入
                }
                ...
                this.running = false
                this.caDisabled = false
                this.$message.error('Rename error: ' + err)
            })
        }
    }
}
