<template>
    <el-dialog title="New Folder" :visible.sync="visible" :close-on-click-modal="false" @close="onClose">
        <el-form :model="folderModel" ref="frmModel" :rules="rules" label-width="120px" label-position="right">
            ...
            <!--<el-form-item prop="LocalizedName" label="本地化名称">
                <el-input v-model="folderModel.LocalizedName" ></el-input>
            </el-form-item>-->
        </el-form>
        ...
        methods: {
            ...
            submit: function (formName) {
                this.$refs[formName].validate((valid) => {
                    ...
                    var _this = this
                    var args = [node.Type, node.ID, this.folderModel.Name]
                    $runtime.channel.invoke('sys.DesignService.NewFolder', args).then(res => {
                        store.tree.onNewNode(res) // 根据返回结果刷新或添加新节点
                        _this.$message.success('Create folder succeed')
                        _this.visible = false
